# FlowForge Cloud：支撑 SaaS Copilot 的有状态 SaaS 产品沙箱
...
SaaS Copilot 是它的自然语言服务入口。用户也能通过普通产品页面查看和操作相同对象，因此可以对照“Agent 说已完成”和“产品实际状态”。这同时支持功能咨询、API 支持、订阅账单和企业账户四类业务。
