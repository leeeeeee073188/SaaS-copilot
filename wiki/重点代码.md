...
| `_extract_entities()` | 使用本地规则提取客户、项目、环境、集成、错误码和使用量等实体，避免额外 LLM 调用 |
...
def _vote(self, llm: Dict, emb: Dict, pat: Dict) -> tuple[IntentCategory, float, Dict[str, float]]:
    ...
    # 2. 如果最终是 technical/billing 等兼容泛意图，但 Pattern 命中
    #    integration、reliability 等细粒度意图，会在置信度允许时细化结果
...
```text
intent=integration, group=integration, primary=support, supporting=delivery,renewal,
scores=[support=1.00, delivery=0.54, renewal=0.48, triage=0.10]
```
...
- `TriageAgent` 只注入客户运营问题分诊规范。
- `DeliveryAgent`、`SupportAgent`、`SuccessAgent` 和 `RenewalAgent` 分别注入对应领域分析规范。
- 复合问题并行时，技术和账单两个 Agent 会分别注入各自的 Skills。

以 `TriageAgent` 为例，它继承 `BaseAgent`，并声明：
...
```python
self.agent_type.value  # "triage"
```

对应调用链是：

```text
TriageAgent.handle(req)
  -> BaseAgent._call_llm(req)
  -> BaseAgent._build_system_prompt(req)
  -> SkillManager.prompt_for(req.message, "triage")
  -> Skill.matches(message, "triage")
```

`skills/triage/SKILL.md` 顶部配置了：
...
因此当用户消息命中这些关键词时，该 Skill 会被注入；如果当前 Agent 不是 `triage`，则会因为 `agents` 不匹配而跳过。
